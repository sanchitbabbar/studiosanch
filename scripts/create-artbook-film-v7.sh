...
build_root="/private/tmp/sanch-artbook-film-v7"
  ...
  for position in {1..18}; do
    ...
    if (( image_number == 10 )); then
      ...
      ffmpeg -hide_banner -loglevel error -y -loop 1 -i "$input" \
        -filter_complex "color=c=#dedad5:s=${render_width}x${render_height}:r=${fps}[bg];[0:v]scale=$foreground_width:$foreground_height:force_original_aspect_ratio=decrease:flags=lanczos,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='255*min(1,min(min(X/$feather,(W-X)/$feather),min(Y/$feather,(H-Y)/$feather)))'[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2:format=auto,zoompan=z='$zoom':x='$x_expr':y='$y_expr':d=91:s=${width}x${height}:fps=$fps,fade=t=in:st=0:d=0.32:color=black,fade=t=out:st=3.44:d=0.36:color=black,format=yuv420p[out]" \
...
ffmpeg -hide_banner -loglevel error -y -i "$build_root/landscape/silent.mp4" -i "$build_root/soundtrack.m4a" \
  ...
  "$output_dir/sanch-artbook-film-v7-studio-master.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$build_root/vertical/silent.mp4" -i "$build_root/soundtrack.m4a" \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest -movflags +faststart \
  "$output_dir/sanch-artbook-film-v7-studio-vertical.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$output_dir/sanch-artbook-film-v7-studio-master.mp4" \
  -t 12 -an -c:v libx264 -preset medium -crf 17 -movflags +faststart \
  "$output_dir/sanch-artbook-film-v7-studio-website-loop.mp4"

echo "Created:"
ls -lh "$output_dir"/sanch-artbook-film-v7-*.mp4
