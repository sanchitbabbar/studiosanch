...
build_root="/private/tmp/sanch-artbook-film-v13"
...
render_edit() {
  ...
  for position in {1..18}; do
    ...
    (( image_number == 10 )) && input="$output_dir/assets/sanch-standing-book-studio-v7.png"
    ...
    local focus_weight="if(lt(T,0.48),(0.48-T)/0.48,if(gt(T,3.32),(T-3.32)/0.48,0))"
    (( position == 1 )) && focus_weight="if(gt(T,3.32),(T-3.32)/0.48,0)"
    (( position == 18 )) && focus_weight="if(lt(T,0.48),(0.48-T)/0.48,0)"
    ...
    if (( image_number == 10 )); then
      ...
      local feather=260
      [[ "$name" == "vertical" ]] && feather=420
      ffmpeg -hide_banner -loglevel error -y -loop 1 -i "$input" \
        -filter_complex "color=c=#dcdad8:s=${render_width}x${render_height}:r=${fps}[bg];[0:v]scale=$foreground_width:$foreground_height:force_original_aspect_ratio=decrease:flags=lanczos,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='255*min(1,min(min(X/$feather,(W-X)/$feather),min(Y/$feather,(H-Y)/$feather)))'[fg];[bg][fg]overlay=(W-w)/2:(H-h)/2:format=auto,zoompan=z='$zoom':x='$x_expr':y='$y_expr':d=91:s=${width}x${height}:fps=$fps,curves=all='0/0 0.025/0 0.55/0.535 1/1',eq=contrast=1.02:brightness=-0.002[base];[base]split[sharp][blurbase];[blurbase]gblur=sigma=5:steps=2[blur];[sharp][blur]blend=all_expr='A*(1-($focus_weight))+B*($focus_weight)',format=yuv420p[out]" \
        -map "[out]" -t "$clip_duration" -an -c:v libx264 -preset medium -crf 16 -movflags +faststart "$output"
    else
      ffmpeg -hide_banner -loglevel error -y -loop 1 -i "$input" \
        -filter_complex "[0:v]$pre_filter,zoompan=z='$zoom':x='$x_expr':y='$y_expr':d=91:s=${width}x${height}:fps=$fps,curves=r='0/0 0.025/0 0.55/0.505 1/0.865':g='0/0 0.025/0 0.55/0.502 1/0.86':b='0/0 0.025/0 0.55/0.498 1/0.85',eq=contrast=1.02:brightness=0.006[base];[base]split[sharp][blurbase];[blurbase]gblur=sigma=5:steps=2[blur];[sharp][blur]blend=all_expr='A*(1-($focus_weight))+B*($focus_weight)',format=yuv420p[out]" \
        -map "[out]" -t "$clip_duration" -an -c:v libx264 -preset medium -crf 16 -movflags +faststart "$output"
    fi
  done
  ...
  # The physical SANCH cover is the signature; no graphic card follows it.
  cp "$build_dir/picture.mp4" "$build_dir/silent.mp4"
}
...
ffmpeg -hide_banner -loglevel error -y -i "$build_root/landscape/silent.mp4" -i "$build_root/soundtrack.m4a" \
  ...
  "$output_dir/sanch-artbook-film-v13-focus-transition-master.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$build_root/vertical/silent.mp4" -i "$build_root/soundtrack.m4a" \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest -movflags +faststart \
  "$output_dir/sanch-artbook-film-v13-focus-transition-vertical.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$output_dir/sanch-artbook-film-v13-focus-transition-master.mp4" \
  -t 12 -an -c:v libx264 -preset medium -crf 17 -movflags +faststart \
  "$output_dir/sanch-artbook-film-v13-focus-transition-website-loop.mp4"

echo "Created:"
ls -lh "$output_dir"/sanch-artbook-film-v13-*.mp4
