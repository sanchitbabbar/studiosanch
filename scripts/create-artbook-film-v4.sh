...
build_root="/private/tmp/sanch-artbook-film-v4"
...
render_edit() {
  ...
  for position in {1..18}; do
    ...
    ffmpeg -hide_banner -loglevel error -y -loop 1 -i "$input" \
      -vf "scale=$render_width:$render_height:force_original_aspect_ratio=increase:flags=lanczos,crop=$render_width:$render_height,zoompan=z='$zoom':x='$x_expr':y='$y_expr':d=91:s=${width}x${height}:fps=$fps,eq=contrast=1.04:brightness=-0.008,fade=t=in:st=0:d=0.32:color=black,fade=t=out:st=3.44:d=0.36:color=black,format=yuv420p" \
...
ffmpeg -hide_banner -loglevel error -y -i "$build_root/landscape/silent.mp4" -i "$build_root/soundtrack.m4a" \
  ...
  "$output_dir/sanch-artbook-film-v4-stabilized-master.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$build_root/vertical/silent.mp4" -i "$build_root/soundtrack.m4a" \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest -movflags +faststart \
  "$output_dir/sanch-artbook-film-v4-stabilized-vertical.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$output_dir/sanch-artbook-film-v4-stabilized-master.mp4" \
  -t 12 -an -c:v libx264 -preset medium -crf 17 -movflags +faststart \
  "$output_dir/sanch-artbook-film-v4-stabilized-website-loop.mp4"

echo "Created:"
ls -lh "$output_dir"/sanch-artbook-film-v4-*.mp4
