...
build_root="/private/tmp/sanch-artbook-film-v5"
...
ffmpeg -hide_banner -loglevel error -y -i "$build_root/landscape/silent.mp4" -i "$build_root/soundtrack.m4a" \
  ...
  "$output_dir/sanch-artbook-film-v5-full-standing-book-master.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$build_root/vertical/silent.mp4" -i "$build_root/soundtrack.m4a" \
  -map 0:v:0 -map 1:a:0 -c:v copy -c:a aac -b:a 256k -shortest -movflags +faststart \
  "$output_dir/sanch-artbook-film-v5-full-standing-book-vertical.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$output_dir/sanch-artbook-film-v5-full-standing-book-master.mp4" \
  -t 12 -an -c:v libx264 -preset medium -crf 17 -movflags +faststart \
  "$output_dir/sanch-artbook-film-v5-full-standing-book-website-loop.mp4"

echo "Created:"
ls -lh "$output_dir"/sanch-artbook-film-v5-*.mp4
