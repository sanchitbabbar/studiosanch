  ...
  return (
    <>
      ...
      <div className={`production-contact-modal${contactOpen ? ' is-open' : ''}`} aria-hidden={!contactOpen}>
        ...
        <section className="production-contact-panel" role="dialog" aria-modal="true" aria-labelledby="production-contact-title">
          ...
          <div className="production-contact-header">
            ...
            <button type="button" onClick={closeContact} aria-label={fr ? 'Fermer' : 'Close'}>×</button>
          </div>
          {contactStatus === 'sent' ? (
            ...
            <>
              <h2 id="production-contact-title">{fr ? 'Parlez-nous de votre idée.' : 'Tell us about your idea.'}</h2>
              <p className="production-contact-intro">{fr ? 'Quelques lignes suffisent pour commencer.' : 'A few lines are enough to begin.'}</p>
      ...
      <style>{`
        ...
        .productions-pill span { display: grid; width: 1.55rem; height: 1.55rem; place-items: center; border: 1px solid rgba(255,255,255,.14); border-radius: 50%; background: rgba(255,255,255,.055); color: rgba(255,255,255,.75); font-size: .7rem; transition: color .4s ease, background .4s ease; }
        ...
        .production-contact-panel { position: relative; width: min(520px,100%); max-height: calc(100vh - 2rem); padding: clamp(1.5rem,3vw,2.25rem); overflow: hidden; border: 1px solid rgba(255,255,255,.15); border-radius: 2rem; background: linear-gradient(145deg, rgba(30,30,30,.88), rgba(4,4,4,.93)); box-shadow: inset 0 1px 0 rgba(255,255,255,.12), 0 40px 120px rgba(0,0,0,.72); color: #fff; transform: translateY(32px) scale(.975); transition: transform .7s cubic-bezier(.16,1,.3,1); }
        ...
        .production-contact-header button { border: 0; padding: 0; background: transparent; color: rgba(255,255,255,.68); font-size: 1.7rem; font-weight: 200; cursor: pointer; }
        ...
        .production-contact-submit span { display: grid; width: 1.45rem; height: 1.45rem; place-items: center; border: 1px solid rgba(255,255,255,.15); border-radius: 50%; background: rgba(255,255,255,.06); color: rgba(255,255,255,.76); transition: color .35s ease, background .35s ease; }
